...
const eventCode = import React, { useState } from 'react';
...
export default function AdminEvents() {
  ...
  const handleDeleteEvent = async (id: number, name: string) => {
    if (!confirm(\Are you sure you want to delete the event "\"? This action cannot be undone.\)) return;
    try {
      const res = await fetch(\\/events/\\, {
        method: 'DELETE',
      });
        ...
        <form onSubmit={handleCreateEvent} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="date" value={newEventDate} onChange={e => setNewEventDate(e.target.value)} className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-2 text-white focus:outline-none focus:border-green-500" />
            </div>
          </div>
              ...
              {events.length === 0 ? (
                ...
                events.map(ev => (
                  <tr key={ev.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                    ...
                    <td className="p-3">
                      {editingId === ev.id ? (
                        <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} className="bg-slate-800 border border-blue-500 rounded px-2 py-1 text-white" />
                      ) : (
                    ...
                    <td className="p-3 text-right space-x-2">
                      {editingId === ev.id ? (
                        <>
                          <button onClick={() => saveEdit(ev.id)} className="bg-green-600 hover:bg-green-500 text-white px-3 py-1 rounded text-xs font-semibold">Save</button>
                        ...
                        <>
                          <button onClick={() => startEdit(ev)} className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs font-semibold">Edit</button>
...
const musicianCode = import React, { useEffect, useState } from 'react';
...
export default function AdminMusicians() {
  ...
  const fetchMusicians = async () => {
    try {
      const res = await fetch(\\/musicians\);
      if (res.ok) {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(\Are you sure you want to delete musician "\"? This will also remove them from all lineups and selections.\)) return;
    try {
      const res = await fetch(\\/musicians/\\, { method: 'DELETE' });
      if (res.ok) {
        fetchMusicians();
      } else {
              ...
              musicians.map(m => (
                <tr key={m.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                  ...
                  <td className="p-3 text-right space-x-2">
                    {editingId === m.id ? (
                      <>
                        <button onClick={() => saveEdit(m.id)} className="bg-green-600 hover:bg-green-500 text-white px-3 py-1 rounded text-xs font-semibold">Save</button>
                        <button onClick={() => setEditingId(null)} className="bg-gray-600 hover:bg-gray-500 text-white px-3 py-1 rounded text-xs font-semibold">Cancel</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEdit(m)} className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs font-semibold">Edit</button>
...
fs.writeFileSync('C:/Users/gusta/OneDrive/web/Made4Jam/frontend/src/pages/AdminEvents.tsx', eventCode);
fs.writeFileSync('C:/Users/gusta/OneDrive/web/Made4Jam/frontend/src/pages/AdminMusicians.tsx', musicianCode);
